...
def standardize_color(color, color_config=None):
    """
...
class StandardizationConfig:
    ...
    auto_resize_textboxes: bool = True
    ...
    resize_skip_pages: Optional[List[int]] = None  # 0-indexed page numbers to skip
    
    # Auto-align annotations
    align_annotations: bool = True
    align_horizontal: bool = True
    align_vertical: bool = True
    horizontal_tolerance: float = 1.0  # points
    vertical_tolerance: float = 10.0    # points
    align_skip_pages: Optional[List[int]] = None  # 0-indexed page numbers to skip
    
    # XFDF color workflow
    apply_xfdf_colors: bool = True  # Apply standardized colors via XFDF import
    
    # Quote normalization
    normalize_quotes: bool = True  # Normalize quotes around annotation values
    
    # Color configuration (optional, for custom standard colors)
    color_config: Optional[Dict[str, Any]] = None  # Dict with standard_red_color, standard_blue_color, standard_green_color
            ...
            for page_num in range(len(doc)):
                ...
                for annot in annots_on_page:
                    try:
                        ...
                        font_color = standardize_color(original_font_color, self.config.color_config)
                        ...
                        if annot_type == fitz.PDF_ANNOT_FREE_TEXT:
                            try:
                                ...
                                skip_this_page = (
                                    self.config.resize_skip_pages is not None 
                                    and page_num in self.config.resize_skip_pages
                                )
                                if self.config.auto_resize_textboxes and content and RESIZER_AVAILABLE and not skip_this_page:
                                    from .text_dimension_calculator import TextDimensionCalculator
                                ...
                                annot.update(
                                    ...
                                    text_color=font_color,  # Use standardized color
            ...
            if self.config.align_annotations and ALIGNER_AVAILABLE:
                try:
                    ...
                    for page_num in range(len(doc)):
                        ...
                        skip_this_page = (
                            self.config.align_skip_pages is not None 
                            and page_num in self.config.align_skip_pages
                        )
                        if skip_this_page:
                            continue
